...
const getPrivateDocRef = (database, uid, collectionName, docId) => {
  if (!uid) return null;
  return doc(database, 'artifacts', appId, 'users', uid, collectionName, docId);
};

const getMapChunksRef = (database, uid) => {
  if (!uid) return null;
...
const Tab3TargetsMap = ({ 
  ...
  const { areaMap, equipmentDB } = settings;
  ...
  useEffect(() => {
    ...
    const mapChunksRef = getMapChunksRef(db, userId);
    const q = query(mapChunksRef, orderBy('index'));
    const unsubscribeMap = onSnapshot(q, (snapshot) => {
      if (snapshot.empty) {
        setMapImageUrl(null);
        return;
      }
  ...
  const uploadMapAsChunks = async (file) => {
    ...
    try {
      ...
      const oldSnapshot = await getDocs(mapRef);
      oldSnapshot.forEach((doc) => {
        batch.delete(doc.ref);
      });

      chunks.forEach((chunk, index) => {
        const docRef = doc(mapRef, `chunk_${index}`);
...
const Tab4Record = ({ appData, updateUnit, addDoc, db, userId, exportToExcel, getUnitCollectionRef }) => {
      ...
      case 'record':
        return (
          <Tab4Record
            ...
            userId={userId}
            exportToExcel={exportToExcel}
            getUnitCollectionRef={getUnitCollectionRef}
          />
        );
      case 'settings':
        return (
