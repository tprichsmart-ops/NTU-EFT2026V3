  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isChangingPassword) {
         ...
         setGlobalMessage({ text: '密碼修改成功！', type: 'success' });
         onClose();
      } else {
      ...
      if (error.code === 'auth/requires-recent-login') msg = '請重新登入後再試一次';
      setLocalError(msg); 
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 relative">
        ...
        <h2 className="text-2xl font-bold text-slate-800 mb-2 flex items-center justify-center">
            {isChangingPassword ? <KeyRound className="w-6 h-6 mr-2 text-indigo-600" /> : <Shield className="w-6 h-6 mr-2 text-indigo-600" />}
            {isChangingPassword ? '修改密碼' : '員工登入'}
        </h2>
        <p className="text-center text-slate-500 mb-6 text-sm">
            {isChangingPassword ? '請輸入您的新密碼' : '請輸入帳號 (例如: 00095)'}
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {isChangingPassword && (
              <div>
                ...
                <input type="password" required value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="••••••" minLength={6} />
              </div>
          )}
          ...
          <button type="submit" disabled={isLoading} className={`w-full py-2.5 rounded-lg text-white font-bold transition flex items-center justify-center ${isLoading ? 'bg-indigo-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-500/30'}`}>{isLoading ? <Loader className="w-5 h-5 animate-spin" /> : (isChangingPassword ? '確認修改' : '登入')}</button>
        </form>
        
        {!isChangingPassword && auth.currentUser && (
             <div className="mt-4 text-center">
                 <button type="button" onClick={() => { setIsChangingPassword(true); setLocalError(''); }} className="text-sm text-indigo-600 hover:underline">修改密碼</button>
             </div>
        )}
        
        {isChangingPassword && (
            <div className="mt-4 text-center">
                <button type="button" onClick={() => { setIsChangingPassword(false); setLocalError(''); }} className="text-sm text-slate-500 hover:underline">返回登入</button>
            </div>
        )}
      </div>
    </div>
...
const App = () => {
  ...
  // Use Public settings for simplicity in this version, or user specific if logged in
  const getSettingsDocRef = useCallback(
    (database, uid) => {
        // Shared settings for everyone to see map, but only writable if auth rules allow
        return doc(database, 'artifacts', appId, 'public', 'data', 'shared_settings');
    },
    []
  );
  
  const getMapChunksRef = useCallback(
      (database) => collection(database, 'artifacts', appId, 'public', 'data', 'map_chunks'),
      []
  );
    ...
    try {
      ...
      const unsubscribe = onAuthStateChanged(authentication, (user) => {
        if (user) {
          ...
          setUserId(null);
          setUserEmail(null);
          // Fallback to anonymous only if needed, but for this request we prefer explicit login
          signInAnonymously(authentication).catch(() => {});
        }
        setIsLoading(false);
      });
  ...
  useEffect(() => {
    ...
    const unsubscribeUnits = onSnapshot(
      ...
      (error) => { console.log("Units listener error (likely permission):", error.message); }
    );

    // Listener for Settings Data
        ...
        if (docSnap.exists()) {
          ...
          setAppData((prev) => ({
            ...
            settings: {
              ...
              uploadedMapUrl: data.uploadedMapUrl || initialSettings.uploadedMapUrl,
            },
            schedules: data.schedules || [],
            meetings: data.meetings || [],
          }));
        } else {
           // If doc doesn't exist, use initial. 
           // Only try to create if logged in (userId exists) to avoid permission errors
           if(userId) {
               setDoc(settingsDocRef, {
                 ...initialSettings,
                 schedules: [],
                 meetings: [],
               }, {merge: true}).catch(e => console.log("Init settings failed:", e.message));
           }
        }
      },
      (error) => console.log('Settings listener error:', error.message)
    );
  ...
  const updateSharedData = async (fields) => {
    ...
    try {
      const docRef = getSettingsDocRef(db, userId);
  ...
  const deleteUnits = async (ids) => {
    ...
    try {
      ...
      setGlobalMessage({ text: `刪除失敗: ${e.message}`, type: 'error' });
    }
  };
  ...
  const uploadMapAsChunks = async (file) => {
    ...
    try {
      ...
      // Trigger a reload of map URL if needed, or rely on listener if we stored meta
      // For now, let's also update the settings doc to trigger a refresh visually
      // In a real chunks impl, we'd read chunks back. For compatibility with existing code:
      // We will read chunks in Tab3 component
  ...
  const Tab1Calendar = () => {
    ...
    const deleteSelectedSchedules = () => {
      ...
      updateSharedData({ schedules: updatedSchedules });
    ...
    const deleteSelectedMeetings = () => {
      ...
      updateSharedData({ meetings: updatedMeetings });
    ...
    const AddScheduleForm = () => {
      ...
      const handleAdd = () => {
        ...
        updateSharedData({ schedules: [...appData.schedules, newSchedule] });
    ...
    const AddMeetingForm = () => {
      ...
      const handleAdd = () => {
        ...
        updateSharedData({ meetings: [...appData.meetings, newMeeting] });
  ...
  const Tab2Guidelines = () => {
    ...
    const handleUpdateSettings = async (field, value) => {
      await updateSharedData({ [field]: value });
      setEditingId(null);
    };
  ...
  const Tab3TargetsMap = () => {
    ...
    // Read chunks to display map
    useEffect(() => {
        if(!db) return;
        const q = query(getMapChunksRef(db), orderBy('index'));
        const unsubscribe = onSnapshot(q, (snapshot) => {
            if (snapshot.empty) { setMapImageUrl(uploadedMapUrl); return; } // Fallback
        ...
        return () => unsubscribe();
    }, [db, uploadedMapUrl, getMapChunksRef]);
    ...
    return (
      <div className="space-y-8 p-6 max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100">
            ...
            <div className="flex space-x-2 items-center">
              ...
              <label className={`${styles.btnPrimary} cursor-pointer flex items-center bg-indigo-600 hover:bg-indigo-700`}>
                <input
                  ...
                  accept="image/*"
                  className="hidden"
                  onChange={(e) => e.target.files?.[0] && uploadMapAsChunks(e.target.files[0])}
                />
          ...
          >
            {isMapLoading ? (
                <div className="flex items-center justify-center h-full text-slate-400">載入地圖中...</div>
            ) : (
                <div
          ...
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-mono">ID: {userId ? (userEmail || userId).split('@')[0] : 'Guest'}</p>
              </div>
            </div>
            
            {userId ? (
                <button onClick={() => { getAuth().signOut(); setUserId(null); setUserEmail(null); }} className={`${styles.btnSecondary} flex items-center shadow-sm`}>
                    <LogOut className="w-4 h-4 mr-2" /> 登出
                </button>
            ) : (
                <button onClick={() => setIsLoginModalOpen(true)} className={`${styles.btnPrimary} flex items-center bg-indigo-700 hover:bg-indigo-800 text-white shadow-md`}>
                    <LogIn className="w-4 h-4 mr-2" /> 員工登入
                </button>
            )}
          </div>
        </div>
